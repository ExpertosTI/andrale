...
const loadOptimizedVideo = (videoEl, tinySrc, hqSrc) => {
    if (!videoEl) return;
    logger(`Starting fetch for ${tinySrc}`);
    fetch(tinySrc)
        .then(response => response.blob())
        .then(blob => {
            const tinyUrl = URL.createObjectURL(blob);
            videoEl.src = tinyUrl;
            logger(`Tiny video loaded: ${tinySrc}`);
            
            // Wait for tiny to be playable before fetching HQ
            videoEl.addEventListener('canplaythrough', () => {
                logger(`Fetching HQ: ${hqSrc}`);
                fetch(hqSrc)
                    .then(r => r.blob())
                    .then(hqBlob => {
                        ...
                        logger(`HQ video swapped: ${hqSrc}`);
                    })
                    .catch(e => logger(`Failed to load HQ: ${hqSrc} - ${e.message}`));
            }, { once: true });
        })
        .catch(e => logger(`Failed to load tiny: ${tinySrc} - ${e.message}`));
};
